...
class Data(object):
    ...
    def _load(self):
        price = pd.read_csv(f"{URL}//data//{self.index_universe}_data.csv", index_col=0, parse_dates=True)
        price.index = price.index.tz_localize(None)
        price = price.reindex(self.schedule.training_dates)
        self.classification = pd.read_csv(f"{URL}//data//{self.index_universe}_classification.csv")
        self.price = price
