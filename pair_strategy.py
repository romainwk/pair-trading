...
class Data(object):
    ...
    def run(self):
        data = pd.read_csv(f"{LOCAL_PATH}/{self.index_universe}_data.csv", index_col=0, parse_dates=True)
        data.index =data.index.tz_localize(None)
        sectors = pd.read_csv(f"{LOCAL_PATH}/{self.index_universe}_sector.csv", index_col=0)
...
class PairScreening(object):
    ...
    def _aggregate(self, eligible_pairs):
        ...
        eligible_pairs.to_excel(f"{LOCAL_PATH}\\eligible_pairs.xlsx")
...
class PairSelection(object):
    ...
    def _load_eligible_pairs(self):
        eligible_pairs = pd.read_excel(f"{LOCAL_PATH}\\eligible_pairs.xlsx", index_col=[2,3,4])[["level_0", "Correlation"]].rename(dict(level_0="GIC_sector",), axis=1)
    ...
    def _get_portfolio_pnl(self):
        ...
        # self.I.to_excel(f"{LOCAL_PATH}\\index_test4.xlsx")
...
class WebApp(object):
    ...
    def results(self):
        ...
        df = pd.read_excel(f"{LOCAL_PATH}\\index_test4.xlsx", index_col=0, parse_dates=True)
        st.line_chart(df)
